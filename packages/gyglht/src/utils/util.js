...
const util = {
    ...
    downLoad: async (guid) => {
        const getToken = (guid) => {
            return new Promise((resolve, reject) => {
                axios.post(this.downSrc() + '/file-web/token/grantToken', {
                    bindGuid: guid,
                    bindType: 1,
        ...
        if (token) {
            window.location.href = `${this.downSrc()}/file-web/downloadFile?guid=${guid}&token=${token}`
        }
    },
    deepClone(target) {
        // 定义一个变量
    ...
    uploadingUrl() {
        return config.requestUrl + "/gt-jy-qyglht/api/"
