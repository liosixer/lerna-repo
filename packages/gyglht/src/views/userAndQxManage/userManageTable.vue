...
<!--                <el-button type="text" v-if="scope.row.creator == 'adminDefault'" class="operation-button"  size="mini" @click="zbrdbXqFun(scope.row.guid)"><i class="iconfont icon-anniutubiao-"><span>查看</span></i></el-button>-->
                <el-button type="text" v-if="scope.row.creator == 'adminDefault'" class="operation-button"  size="mini" @click="zbrdbFpqxFun(scope.row.guid,'readOnly')"><i class="iconfont icon-anniutubiao-"><span>查看</span></i></el-button>
                <template v-if="scope.row.creator !== 'adminDefault'">
                  <el-button type="primary" plain class="operation-button" size="mini" @click="zbrdbBjFun(scope.row)"><i class="iconfont icon-bianji2"><span>编辑</span></i></el-button>
                  <el-button type="info" plain class="operation-button" size="mini" @click="zbrdbDeletFun(scope.row.guid)"><i class="iconfont icon-shanchu1"><span>删除</span></i></el-button>
                  <el-button type="primary" plain class="operation-button" size="mini" @click="zbrdbFpqxFun(scope.row.guid)"><i class="iconfont icon-fenpeiquanxian"><span>分配权限</span></i></el-button>
                </template>
              </el-button-group>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    zbrdbFpqxFun(id,readOnly) {
      this.$router.push({ path: "/userAndQxManage/userManageQx", query: { id , readOnly} });
    },
