    ...
    <div class="contentDiv">
      <el-form :inline="true" :model="checkData" :rules="checkRuleData" ref="checkData" class="checkForm" label-position="left">
        <el-form-item label="用户名:" prop="accountName">
          <el-input placeholder="请输入您的用户名" v-model="checkData.accountName" @blur="jyzhPpFunc"></el-input>
        </el-form-item>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      checkRuleData: {
        accountName: [{ required: true, message: "请输入您的用户名", trigger: "change" }],
        ...
        yzm: [{ required: true, message: "请输入正确的手机验证码", trigger: "blur" }],
      },
      agreeXy: [],
      formData: {
        accountName: "",
  ...
  methods: {
    ...
    async isUserNameExists(isZc) {
      ...
      const res = await this.portService
        .isUserNameExists(formData)
        .then((res) => {
          if (res.code == 0 && this.checkData.accountName != "") {
            this.$message({
              type: "warning",
              center: true,
              duration: 2000,
    ...
    async reModifyPassword() {
      ...
      const res = await this.portService
        .reModifyPassword(this.formData)
        .then((res) => {
          if (res.code == 1) {
            this.$message({
              type: "success",
    ...
    async login() {
      ...
      await axios
        .post(`${this.requestUrl}/guotou-auth/oauth/token`, qs.stringify(this.loginForm), {
          headers: {
      ...
      if ((theUA.match(/msie\s\d+/) && theUA.match(/msie\s\d+/)[0]) || (theUA.match(/trident\s?\d+/) && theUA.match(/trident\s?\d+/)[0])) {
        ...
        if (ieVersion < 12) {
          ...
          if(this.$route.query.from == 'bjs'){
            this.$router.push({ path: "/zhAccount" ,query:{from:'bjs'}});
          }else{
