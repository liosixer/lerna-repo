    ...
    <section class="container">
        <div class="headerDiv">
            <ul>
                ...
                <li>
                    <span @click="ckssjdFunc">查看申诉进度</span>
        ...
        <div class="contentDiv">
            <el-form :inline="true" :model="checkData" :rules="checkRuleData" ref="checkData" class="checkForm" label-position="left">
                ...
                <el-form-item label="重置密码:" prop="password">
                    <el-input type="password" placeholder="请设置您申诉后的登录密码" v-model="checkData.password"></el-input>
                </el-form-item>
                ...
                <el-form-item label="申诉材料:" prop="fileId">
                    <el-input placeholder="申诉材料" v-model="checkData.fileId" style="display:none;"></el-input>
                    <Uploader action="/upload" :buttonTxt="buttonTxt" @sucData="handleSucDataFddbrShenFenZhengBH" :noToken="true"></Uploader>
                    <el-button type="text" v-if="!checkData.fileId" @click="downTemplate">账号申诉申请表模板.doc</el-button>
                </el-form-item>
                <el-form-item>
                    <el-button type="primary" @click="loginBtn(checkData)">提交申诉材料</el-button>
                </el-form-item>
            </el-form>
        </div>
        <div class="footerDiv">
...
<script>
    ...
    export default {
        name: 'zhangHaoShengSu',
        components: { userAgreementDialog , Uploader},
        data() {
            return {
                checkData: {
                    qiYeName: "",
                ...
                checkRuleData:{
                    qiYeName: [
                        { required: true, message: '请输入正确企业名称！', trigger: 'blur' }
                    ], 
                    ...
                    fileId: [
                        { required: true, message: '请先下载专用账号申诉申请表，填写盖章后再上传', trigger: 'blur' }
                    ]
                },
                agreeXy: [],
                formData:{
        ...
        methods: {
            handleSucDataFddbrShenFenZhengBH (payload) {
                this.checkData.fileId= payload.guid
            },
            //密码再次校验
            ...
            async zhangHaoShengSu() {
                this.formData={
                    ...
                    fileId: this.checkData.fileId
                }
                const res = await this.portService.zhangHaoShengSu(this.formData)
                .then(res => {
                    if(res.code==1){
                        this.$message({
                            type: "success",
            ...
            ckssjdFunc(){
                if(this.$route.query.from == 'bjs'){
                    this.$router.push({ path: "/accountJd",query:{from:'bjs'} })
                }else{
                    this.$router.push({ path: "/accountJd" })
                }
            ...
            downTemplate(){
                if(this.$route.query.from==='bjs'){
                    window.location.href="./账号申诉模板-北交所.docx"
                }else{
                    window.location.href="./账号申诉模板.docx"
                }
            }
        },
        create() {},
