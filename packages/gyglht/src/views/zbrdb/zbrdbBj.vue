  ...
  <section class="container page-detail">
    <div class="zbrdbAddHeader global-title">
      <PageCrumb :d="{ breadcrumbList: breadcrumbList }">
        <div slot="btns">
          <el-button type="primary" @click="saveFunc(zbrdbData)" size="small" v-load="!canpost" :loading="saveLoading">保存</el-button>
          <el-button @click="$router.go(-1)" size="small">返回</el-button>
        </div>
      </PageCrumb>
      <!-- <div class="global-title-box">
                <span class="global-title-name">招标人代表-编辑</span>
                <div class="global-title-btns fr">
                    <el-button type="primary" @click="saveFunc(zbrdbData)" size="small">保存</el-button>
                    <el-button @click="$router.go(-1)" size="small">返回</el-button>
                </div>
            </div> -->
    </div>
    <div class="zbrdbAddContent global-content">
      <el-card>
        ...
        <div class="global-card-content">
          <el-form :model="zbrdbData" :rules="zbrdbRuleData" size="small" ref="zbrdbData" label-width="100px" label-position="right">
            <el-row :gutter="20">
              <el-col :span="12">
            ...
            <el-row :gutter="20">
              ...
              <el-col :span="12">
                <el-form-item label="银行联行号:" prop="bank_lianhanghao">
                  <el-input placeholder="请输入银行联行号" v-model="zbrdbData.bank_lianhanghao"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row :gutter="20">
              <el-col :span="12">
                <el-form-item label="是否有效:" prop="statusVal">
                  <el-select v-model="zbrdbData.statusVal" placeholder="请选择">
                    <el-option v-for="item in zbrdbData.status" :key="item.value" :label="item.label" :value="item.value"> </el-option>
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item label="招标人代表编号:" prop="zbrdaibiao_code">
                  <el-input placeholder="自动获取" readonly v-model="zbrdbData.zbrdaibiao_code"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row :gutter="20">
              <el-col :span="12">
                <el-form-item label="备注:" style="width: 100%">
                  <el-input type="textarea" :rows="4" autosize placeholder="请输入" v-model="zbrdbData.beizhu" style="width: 100%"> </el-input>
                </el-form-item>
              </el-col>
            </el-row>
          </el-form>
      ...
      <el-card>
        <div slot="header" class="clearfix">
          <span class="global-card-header-name">招标人代表共享</span>
          <el-button type="primary" size="small" @click="addGxQy" style="float:right;">添加</el-button>
        </div>
        <div class="global-card-content">
            ...
            <el-table-column fixed="right" label="操作" min-width="100" align="left">
              <template slot-scope="scope">
                <el-button-group class="operation-button-group">
                  <el-button type="info" plain class="operation-button" size="mini" @click="deleteGxQy(scope.$index, zbrdbGxData)">
  ...
  data() {
    return {
      ...
      zbrdbRuleData: {
        ...
        // bank_lianhanghao: [
        //   { required: true, message: "请输入银行联行号", trigger: "blur" },
        //   { pattern: /^[\d]{6,13}$/g, message: "请输入6-13位数字的银行联行号", trigger: "blur" },
        // ],
      },
      zbrdbGxData: [],
      canpost: true,
  ...
  methods: {
    ...
    async queryZhaoBiaoRenDaiBiaoInfo() {
      ...
      if (res.data) {
        ...
        this.zbrdbData.bank_lianhanghao = thisData.bank_lianhanghao;
        this.zbrdbData.zbrdaibiao_code = thisData.zbrdaibiao_code;
    ...
    async saveOrUpdateZhaoBiaoRenDaiBiao() {
      ...
      this.addData = {
        ...
        guid: this.ckGuid,
        bank_lianhanghao: this.zbrdbData.bank_lianhanghao,
        zbrdaibiao_code: this.zbrdbData.zbrdaibiao_code,
      };
      const res = await this.portService
        ...
        .then((res) => {
          if (res.code == 1) {
            ...
            this.$router.push({ path: "/zbrdb/zbrdbTable" });
          }
        })
        .catch((error) => {});
      this.canpost = true;
      ...
      const res = await this.portService
        ...
        .then((res) => {
          if (res.code == 0) {
            ...
            this.$confirm("确定保存招标人代表修改信息吗?", "提示", {
