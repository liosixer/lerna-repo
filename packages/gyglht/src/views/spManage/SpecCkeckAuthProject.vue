            ...
            <div class="global-content-detail">
                <self-table border :data="zzxxData" header-cell-class-name="headercell" style="width:100%;margin:0;"
                    ...
                    <el-table-column prop="qiYe_BH" label="企业代码" min-width="150px"
                                     show-overflow-tooltip></el-table-column>
                    <el-table-column prop="auth_status_name" label="是否需要二级单位审核" min-width="120px">
                        <template slot-scope="scope">
                            <span v-if="scope.row.projectApprovalAppStatus == 0 || scope.row.projectApprovalAppStatus == null">不需要</span>
                            <span v-else-if="scope.row.projectApprovalAppStatus == 1">需要</span>
                        </template>
                    </el-table-column>
                    <el-table-column label="操作" min-width="120" align="left">
                        <template slot-scope="scope">
                            <el-button-group class="operation-button-group">
                                <el-button v-if="scope.row.projectApprovalAppStatus == 0 || scope.row.projectApprovalAppStatus == null" type="primary" plain
                                           class="operation-button" size="mini"
                                           @click="setAuth(scope.row.userQiYe_Guid,1,'projectApproval')">
                                    <i class="iconfont icon-ziyuan"><span>设置为需要</span></i>
                                </el-button>
                                <el-button v-else type="primary" plain class="operation-button" size="mini"
                                           @click="setAuth(scope.row.userQiYe_Guid,0,'projectApproval')">
                                    <i class="iconfont icon-ziyuan"><span>设置为不需要</span></i>
                                </el-button>
                            </el-button-group>
                        </template>
                    </el-table-column>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                breadcrumbList: [{label: "采购立项权限", link: ""}],
                zzxxData: [],
                total: 0,
                searchForm: {
                    page: 1,
        ...
        methods: {
            ...
            async setAuth(id, type,shenheType) {
                let mes;
                if(shenheType == 'projectApproval'){
                    if(type == 0){
                        mes = '请您确认是否将采购立项权限设置为不需要二级单位审核！'
                    }else if(type == 1){
                        mes = '请您确认是否将采购立项权限设置为需要二级单位审核！'
                    }
                }
                this.$confirm(
                    mes,
