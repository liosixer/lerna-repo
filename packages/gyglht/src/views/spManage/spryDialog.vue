<template>
    <section class="container">
        <el-dialog title="添加审批人员"
                   :append-to-body="true" :visible.sync="dialogTableVisible" :close-on-click-modal="false">
            <el-form :inline="true" :model="allQiYeData" class="allQiYeForm" label-position="right" size="small">
...
<!--                </el-form-item>-->
            ...
            <self-table border :data="allQiYeTableData" height="422px" @selection-change="handleSelectionChange"
...
<!--                <el-table-column prop="iDCard" label="身份证号码" min-width="150px" show-overflow-tooltip-->
<!--                                 align="right"></el-table-column>-->
                <el-table-column prop="roleName" label="角色" min-width="150px" show-overflow-tooltip></el-table-column>
                <el-table-column prop="departName" label="所属部门" min-width="150px"
...
<script>
    ...
    export default {
        ...
        methods: {
            //查询所有企业
            async querySubShenPiRenYuan(liuZhuanFanwei) {
                this.searchForm = {
                    userName: this.allQiYeData.userName,
                    // deptName: this.allQiYeData.deptName,
                    page: this.searchForm.page,
                    rows: this.searchForm.rows,
                    liuZhuanFanwei: liuZhuanFanwei
                }
                const res = await this.portService.querySubShenPiRenYuan(this.searchForm)
                if (res.data) {
            ...
            qyBoolen(liuZhuanFanwei) {
                this.dialogTableVisible = true
                let liuZhuanFanweiparam = ''
                liuZhuanFanweiparam = liuZhuanFanwei && liuZhuanFanwei.length > 0 ? liuZhuanFanwei.join() : ''
                this.querySubShenPiRenYuan(liuZhuanFanweiparam)
            }
        },
        create() {
        },
