            ...
            <div class="global-content-detail">
                <self-table border :data="zzxxData" header-cell-class-name="headercell" style="width:100%;margin:0;"
                    ...
                    <el-table-column prop="qiYe_BH" label="企业代码" min-width="150px"
                                     show-overflow-tooltip></el-table-column>
                    <el-table-column prop="auth_status_name" label="采购计划终审权限" min-width="120px">
                        <template slot-scope="scope">
                            <span v-if="scope.row.yearPlanAppStatus == 0 || scope.row.yearPlanAppStatus == null">无</span>
                            <span v-else-if="scope.row.yearPlanAppStatus == 1">有</span>
                        </template>
                    </el-table-column>
                    <el-table-column label="操作" min-width="120" align="left">
                        <template slot-scope="scope">
                            <el-button-group class="operation-button-group">
                                <el-button v-if="scope.row.yearPlanAppStatus ==1" type="primary" plain
                                           class="operation-button" size="mini"
                                           @click="setAuth(scope.row.userQiYe_Guid,0,'yearPlan')">
                                    <i class="iconfont icon-ziyuan"><span>设置为无</span></i>
                                </el-button>
                                <el-button v-else type="primary" plain class="operation-button" size="mini"
                                           @click="setAuth(scope.row.userQiYe_Guid,1,'yearPlan')">
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                breadcrumbList: [{label: "采购计划权限", link: ""}],
                zzxxData: [],
                total: 0,
                searchForm: {
                    page: 1,
        ...
        methods: {
            ...
            async setAuth(id, type,shenheType) {
                let mes;
                if(shenheType == 'yearPlan'){
                    if(type == 0){
                        mes = '请您确认是否将采购计划终审权限设置为无！'
                    }else if(type == 1){
                        mes = '请您确认是否将采购计划终审权限设置为有！'
                    }
                }
                this.$confirm(
                    mes,
