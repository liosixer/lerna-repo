...
if (!regDev.test(process.env.NODE_ENV)) {
    // baseURL ="./GT_API"; // 正式环境
    baseURL = config.requestUrl+'/center-web-api' // 正式环境
}
			...
			if (auth) {
				...
				if (isTokenExpired()) {
		 			...
		 			if (!window.isRefreshing) {
						...
						console.log('-----1-----')
						 /*发起刷新token的请求*/
						axios.post(`${szjyjt8083()}/guotou-auth/oauth/token`, qs.stringify(loginForm),{
							headers: {
						...
						/*将标志置为false*/
							...
							axios.get(`${szjyjt8083()}/guotou-auth/resource/userInfo` ,{
								headers: {
    ...
    (response) => {
        if (response.status === 200) {
            if (response.data.code == -1 || response.data.code == 0) {
                if (response.config.url == '/reg/isUserNameExists' || response.config.url == '/reg/isZhangHaoExists' ){
                }else{
                    VUE.$message({
                        type: 'error',
