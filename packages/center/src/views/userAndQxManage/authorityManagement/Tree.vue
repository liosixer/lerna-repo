  ...
  <div class="tree_selector" :class="{ tree_drop: drop , 'tree-select-flex': flex }">
    ...
    <div class="tree_selector_box" v-show="show || !drop">
      ...
      <div class="tree-box el-main">
        ...
        >
          <div class="custom-tree-node" slot-scope="{ node, data }" v-if="showctr">
            <span>{{ node.label }}</span>
            <span>
              <i v-if="data.enableStatus == 1" class="el-icon-check custom-tree-abs-i" style="color:green;"></i>
              <i v-else-if="data.enableStatus == 0" class="el-icon-close custom-tree-abs-i" style="color:red"></i>
              <el-button
              ...
              <div class="drop_box" v-if="currentIndex === node.id && showBox === true">
                <div class="drop_title">
                  <i class="el-icon-close" @click.stop="handleCloseBox"></i>
                </div>
                <ul>
                  <li @click.stop="handleAddSubNode(node)">添加子部门</li>
                  <li @click.stop="handleDeleteNode(node)">删除部门</li>
                  <li @click.stop="handleModifyNode(node)">修改部门</li>
...
<script>
import mixin from "@/components/tree/mixin";
export default {
  mixins: [mixin],
  model: {
    prop: "value",
