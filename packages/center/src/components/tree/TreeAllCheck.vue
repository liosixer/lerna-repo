...
export default {
  ...
  methods: {
    ...
    filterParentNodes(nodes, node, parent) {
      // console.log(nodes, node, parent,'nodes, node, parent')
      let data;
      nodes.forEach(item => {
    ...
    handleCheckNode(node, checked) {
      ...
      if (this.strictly) {
        ...
        if (choise && node.children) {
          ...
          let arrNode = this.prossingArray(node.children) //全选多层级数组
          // 全选
          if (children && keys.includes(node.id)) {
             nodes = [...nodes, ...children,...arrNode];
          }
          // 反选
          else if (children) {
            let childrenNodes = [...children,...arrNode].map(i => i.id);
    ...
    prossingArray(nodeChild){
      let newArr = []
      nodeChild.forEach((item,index)=>{
        newArr.push(...item.children)
        item.children.forEach(j=> newArr.push(...j.children))
      })
      return newArr
    },
    // 取消选择
