...
import { removeNullOrEmptyAndTrim } from '@/utils/utils';
...
if (!regDev.test(process.env.NODE_ENV)) {
  // baseURL ="./GT_API"; // 正式环境
  // baseURL = config.requestUrl + '/gt-jy-zhaobiao/api'; // 正式环境
  baseURL = config.requestUrl;
  apiPrefix = '/nmg-jy-zhaobiao';
}
...
service.interceptors.request.use(
  (config) => {
    if (config.data) {
      config.data = removeNullOrEmptyAndTrim(config.data);
    }
    if (config.params) {
      config.params = removeNullOrEmptyAndTrim(config.params);
    }
      ...
      if (auth) {
        ...
        if (isTokenExpired()) {
          ...
          if (!window.isRefreshing) {
            ...
            axios
              .post(
                `${getRequestUrl()}/guotou-auth/oauth/token`,
              ...
              .then((res) => {
                ...
                axios
                  .get(`${getRequestUrl()}/guotou-auth/resource/userInfo`, {
                    headers: {
...
service.interceptors.response.use((response) => {
  if (response.status === 200) {
    ...
    if (config.url.includes('/NM_API') || config.url.includes('/nmg-')) {
