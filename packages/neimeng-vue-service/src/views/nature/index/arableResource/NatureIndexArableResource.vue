    ...
    <el-card class="cus-main-card main-card">
      ...
      <div class="card-content">
        ...
        >
          ...
          <el-table-column label="指标类别" width="100" align="center">
            <template #default="{row}">
              {{ _indexType(row.indexType) }}
            </template>
          </el-table-column>
...
export default {
  name: 'NatureIndexArableResource',
  ...
  computed: {
    ...
    _indexType() {
      return (type) => {
        const obj = this._.find(indexType, { code: type * 1 });
        return obj ? obj.name : '';
      };
    },
  },
  ...
  methods: {
    async getList() {
      ...
      params = { ...params, ...{ systemFrom: 2, type: 10 } };
      if (this.tabValue !== 100) {
        params.systemType = this.tabValue;
      } else {
        params.systemType = null;
      }
